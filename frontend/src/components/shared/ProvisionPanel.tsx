...
export default function ProvisionPanel({ nodeId, onClose }: Props) {
  ...
  return (
    <>
      {/* Mobile backdrop */}
      <div className="md:hidden fixed inset-0 bg-black/30 z-50" onClick={onClose} />

      <div className="fixed inset-y-0 right-0 w-full md:w-96 bg-white border-l border-gray-200 shadow-lg z-50 flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 safe-top">
          <h3 className="text-sm font-semibold text-gray-900 truncate flex-1 mr-2">{nodeId}</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 text-lg shrink-0 w-8 h-8 flex items-center justify-center">&times;</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 pb-safe">
          ...
          {error && (
            <div className="text-sm text-gray-600 bg-gray-50 rounded p-3">
              ...
              <p className="text-xs text-blue-600 mt-2 font-mono break-all">{nodeId}</p>
            </div>
          )}
